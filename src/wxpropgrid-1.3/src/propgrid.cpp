...
wxPGGlobalVarsClass::wxPGGlobalVarsClass()
{
    ...
    // Uncover non-exported wxVariant class infos
    wxVariant v;

    v = (long)0;
    wxVariantClassInfo_long = v.GetData()->GetClassInfo();

    v = wxString();
    wxVariantClassInfo_string = v.GetData()->GetClassInfo();

    v = (double)0.0;
    wxVariantClassInfo_double = v.GetData()->GetClassInfo();

    v = (bool)false;
    wxVariantClassInfo_bool = v.GetData()->GetClassInfo();

    v = wxArrayString();
    wxVariantClassInfo_arrstring = v.GetData()->GetClassInfo();

    wxColour col;
    wxVariant v2((wxObject*)&col);
    wxVariantClassInfo_wxobject = v2.GetData()->GetClassInfo();

    wxList list;
    v = wxVariant(list);
    wxVariantClassInfo_list = v.GetData()->GetClassInfo();

#if wxCHECK_VERSION(2,8,0)
    v << *wxRED;
    wxVariantClassInfo_wxColour = v.GetData()->GetClassInfo();
#endif

#if wxUSE_DATETIME
    v = wxVariant(wxDateTime::Now());
    wxVariantClassInfo_datetime = v.GetData()->GetClassInfo();
#endif
...
#ifdef __WXPYTHON__
bool wxPGProperty::StringToValue( wxVariant&, const wxString&, int argFlags ) const
{
    return false;
}
...
wxVariant wxPGProperty::GetDefaultValue() const
{
    ...
    if ( !value.IsNull() )
    {
        wxClassInfo* classInfo = value.GetData()->GetClassInfo();
        ...
        if ( m_value.GetData()->IsKindOf(CLASSINFO(wxPGVariantData)) )
            return wxStaticCast(m_value.GetData(), wxPGVariantData)->GetDefaultValue();
...
bool wxPropertyGrid::DoSelectProperty( wxPGProperty* p, unsigned int flags )
{
    ...
    else
    {
        ...
        if ( m_selected )
        {
            ...
            else // if ( prev->GetY2(m_lineHeight) < (int)m_bottomy )
...
wxObject* wxPG_VariantToWxObject( const wxVariant& variant, wxClassInfo* classInfo )
{
    ...
    if ( vdata->IsKindOf( &wxPGVariantData::ms_classInfo ) )
         return (wxObject*) ((wxPGVariantData*)vdata)->GetValuePtr();

    if ( vdata->GetClassInfo() == wxPGGlobalVars->wxVariantClassInfo_wxobject )
#if wxCHECK_VERSION(2,8,0)
...
long wxPGVariantToInt( const wxVariant& variant )
{
    ...
    if ( variant.GetData()->GetClassInfo() == &wxPGVariantDataLongLong::ms_classInfo )
    {
...
void wxPropertyGridState::DoSetPropertyValues( const wxList& list, wxPGProperty* defaultCategory )
{
                    ...
                    if ( pos2 > 0 && pos2 < (name.size()-1) )
                    {
                        ...
                        if ( entryType == wxT("attr") )
                        {
                            ...
                            if ( foundProp )
                            {
                                ...
                                wxList& list2 = current->GetList();
                                wxList::const_iterator node2;
