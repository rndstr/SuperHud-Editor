...
wxWebUpdatePlatform wxWebUpdatePlatform::GetThisPlatform()
{
    ...
    // FIXME rs
    // the correct way would be to retrieve the portid through:
    // wxGetApp().GetTraits()->GetToolkitVersion(0, 0)
    // but as the WebUpdaterApp is defined in a sourcefile (webapp.cpp)
    // we cannot include this here.. and access it
    // so just assume port based on os (windows GTK won't compile here)
