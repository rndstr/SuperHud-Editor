...
#define wxWU_LOCAL_XMLSCRIPT            wxT("data/webupdater/local.xml")

//! The local XRC file which is loaded if no --xrc option is specified.
#define wxWU_LOCAL_XRC                  wxT("data/webupdater/webupdatedlg.xrc")

//! The name of the XRC resource which is loaded if no --res option is specified.
#define wxWU_XRC_RESOURCE               wxT("wxWebUpdateLogDlg")

//! The name of our default log file.
#define wxWU_LOGFILENAME                wxT("data/webupdater/webupdate.log")
...
public:
    ...
    void CreateFileLogger()
    {
        ...
        // now put logfile into user datadir as there is no option switch to specify path :/
        // (otherwise might puts it in wrong place if user starts the app from somewhere else)
